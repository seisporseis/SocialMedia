...
@section('content')
    <div class="container mx-auto justify-between items-center p-10">
        <div class="flex flex-col xl:flex-row md:w-full justify-center gap-6">      
            ...
            <div class="w-full xl:w-1/2">
                <p class="font-bold text-teal-900 text-lg"> 
                    {{ $post->user->username }} 
                </p>
                ...
                @auth
                <div class="px-2 shadow bg-stone-100 rounded-md">
                    ...
                    <form method="POST" action="{{ route('posts.destroy', $post) }}">
                        @method('DELETE')
                        @csrf
                        <button class="flex gap-2 items-center py-4 text-xs underline cursor-pointer border border-transparent rounded-md text-teal-900 hover:bg-slate-100 hover:shadow-sm ">
                    ...
                    @endif
                @endauth
                <div class="mt-4 justify-center gap-6 ">
                    <div class="shadow bg-stone-100 p-5 mb-5 rounded-md">
                        @auth
                        ...
                        <div class=" my-5 max-h-96 overflow-y-scroll">
                           @if ($post->comments->count())
                            @foreach ($post->comments as $comment)
                                <div class="py-4 border-gray-300 border-b">
                                    <a href="{{ route('posts.index', $comment->user) }}" class="font-bold text-xs hover:text-pink-600 hover:underline">
